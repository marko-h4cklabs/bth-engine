...
import { deployViaRestApi } from '../landing/deployer.js';
...
app.post('/api/deploy', (req: Request, res: Response) => {
  ...
  (async () => {
    ...
    try {
      ...
      sseWrite(res, { type: 'log', text: `[deploy] Deploying ${pageDir} via REST API...\n` });

      const liveUrl = await deployViaRestApi(slug, pageDir, token, (msg) => {
        sseWrite(res, { type: 'log', text: `[deploy] ${msg}\n` });
      });
...
app.post('/api/regen-landing', (req: Request, res: Response) => {
  ...
  (async () => {
    try {
      ...
      const pageDir = resolve(process.cwd(), 'output', 'pages', slug);
      sseWrite(res, { type: 'log', text: `[regen] Deploying ${pageDir} via REST API...\n` });

      const liveUrl = await deployViaRestApi(nSlug, pageDir, token, (msg) => {
        sseWrite(res, { type: 'log', text: `[regen] ${msg}\n` });
      });
